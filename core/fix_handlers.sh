...
auto_fix_storage() {
    read -rp "${YELLOW}Run auto-fix for Storage? [y/N]: ${RESET}" CONFIRM
...
auto_fix_repository() {
    read -rp "${YELLOW}Run auto-fix for Repository? [y/N]: ${RESET}" CONFIRM
...
auto_fix_nodejs() {
    read -rp "${YELLOW}Run auto-fix for NodeJS? [y/N]: ${RESET}" CONFIRM
